...
#endif
/*todo adc handler*/
extern TIM_HandleTypeDef TimerHeaterAndPwm;
        ...
        if (!heating && input < temp) {
          if (ELAPSED(ms, t1 + 5000UL)) {
            ...
            if (cycles > 0) {
              ...
              bias = constrain( bias, 20l, max_pow - 20 );
...
float Temperature::analog2temp(int raw, uint8_t e) {
  ...
  //TODO create adc voltrage varibale for VREF
...
void Temperature::disable_all_heaters() {
  ...
  #define MAX6675_HEAT_INTERVAL 250u
    ...
    #define MAX6675_SPEED_BITS (_BV(SPR1)) // clock Г· 64
  #else
    uint16_t max6675_temp = 2000;
    #define MAX6675_ERROR_MASK 4
    #define MAX6675_DISCARD_BITS 3
    #define MAX6675_SPEED_BITS (_BV(SPR0)) // clock Г· 16
  #endif
...
void TIM3_IRQHandler(void)
{
  ...
  Temperature::isr();
  HAL_TIM_IRQHandler(&TimerHeaterAndPwm);
...
//Почему не сделать в цикле?
  ...
  switch (temp_state) {
    ...
    case MeasureTemp_0:{
      #if HAS_TEMP_0
    	while(HAL_ADC_PollForConversion(&adc1Handler,100)!=HAL_OK);
        	raw_temp_value[0] += HAL_ADC_GetValue(&adc1Handler);
      #endif
