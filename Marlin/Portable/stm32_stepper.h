...
class Stepper {
	...
	private:
		...
		static unsigned short stepperTimerPeriodNominal;
...
#define MIXING_STEPPERS_LOOP(VAR) \
	...
	public:
		...
		//
		static FORCE_INLINE bool motor_direction(AxisEnum axis) {
	...
	private:
		...
		static FORCE_INLINE unsigned short calc_timer(
				unsigned short step_rate) {
			uint16_t timer;

			NOMORE(step_rate, MAX_STEP_FREQUENCY);
			//TODO software pwm limit
			...
			uint16_t TICK_TIMER_PRESCALER = TimerStepper.Init.Prescaler;
			uint32_t timerTickFrequency = 84000000 / TICK_TIMER_PRESCALER;
			if ( step_rate < 20 ) {//minimal stepper frequncy limited
				step_rate = 20;
			}

			timer = ( uint16_t )(timerTickFrequency/step_rate);
			//max stepper limit aprox 100khz
			if ( timer < 20 )
				timer = 20;
			return timer;
		}
...
#endif
			/*Todo Need create timer abstraction*/
			deceleration_time = 0;
			// step_rate to timer interval
			stepperTimerPeriodNominal = calc_timer(current_block->nominal_rate);
			// make a note of the number of step loops required at nominal speed
			step_loops_nominal = step_loops;
			acc_step_rate = current_block->initial_rate;
			acceleration_time = calc_timer(acc_step_rate);
			TimerStepper.Init.Period = acceleration_time;
