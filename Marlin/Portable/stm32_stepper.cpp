...
#endif
#endif
//todo придумать название включени€
...
unsigned short Stepper::stepperTimerPeriodNominal;
...
//todo timer
#define ENABLE_STEPPER_DRIVER_INTERRUPT()  HAL_TIM_Base_Start_IT(&TimerStepper)
#define DISABLE_STEPPER_DRIVER_INTERRUPT() HAL_TIM_Base_Stop_IT(&TimerStepper)
...
void TIM2_IRQHandler() {
	HAL_TIM_IRQHandler(&TimerStepper);
	stepper.isr();
}
...
void Stepper::isr() {
	...
	if ( !current_block ) {
		...
		if ( current_block ) {
			...
			counter_X = counter_Y = counter_Z = counter_E =
					- ( current_block->step_event_count >> 1 );
			...
			if (current_block->steps[Z_AXIS] > 0) {
				enable_z();
				timer.Init.Period = 2000; //1ms wait
				stepperTimer.Instance->ARR= stepperTimer.Init.Period;
				return;
			}
#endif
			...
			stepperTimerPeriodUpdate(2000); //1 khz
		}
	}
...
#define STEP_ADD(AXIS) \
        ...
        if (_COUNTER(AXIS) > 0) { _APPLY_STEP(AXIS)(!_INVERT_STEP_PIN(AXIS),0); }
			...
			STEP_ADD(Z);
			/*TODO delay */
			/*2us delay from 4MHz Timer for drv8825 */
uint32_t usTick = getTimerValue(&Timer_usDelay);
...
while (getTimerValue(&Timer_usDelay) - usTick < DIRPulseDelay);
			STEP_IF_COUNTER(X);
			STEP_IF_COUNTER(Y);
			STEP_IF_COUNTER(Z);
		...
		if ( step_events_completed
			...
			stepperTimerPeriodUpdate(timer);
			...
			eISR_Rate = (stepperTimerNominal >> 2) * step_loops_nominal / abs(e_steps[TOOL_E_INDEX]);

#endif

			stepperTimerPeriodUpdate(stepperTimerPeriodNominal);
			// ensure we're running at the correct step rate, even if we just came off an acceleration
			step_loops = step_loops_nominal;
		}

		stepperTimerPeriodUpdate(
				( TimerStepper.Init.Period < ( TimerStepper.Instance->CNT + 16 ) ?
						( TimerStepper.Instance->CNT + 16 ) : TimerStepper.Init.Period ));
...
#define E_AXIS_INIT(NUM) AXIS_INIT(e## NUM, E## NUM, E)
	...
	/*TODO here been initialization of timers before*/
...
void Stepper::report_positions() {
	CRITICAL_SECTION_START;
	long xpos = count_position[X_AXIS], ypos = count_position[Y_AXIS], zpos =
