...
float Planner::max_feedrate_mm_s[NUM_AXIS], // Max speeds in mm per second
		Planner::axis_steps_per_mm[NUM_AXIS], Planner::steps_to_mm[NUM_AXIS];
...
float Planner::min_feedrate_mm_s,
		...
		Planner::max_z_jerk, Planner::max_e_jerk,
 ...
 */
void Planner::calculate_trapezoid_for_block(block_t* block, float entry_factor,
		float exit_factor) {
	...
	int32_t accelerate_steps = ceil(
			estimate_acceleration_distance(initial_rate, block->nominal_rate,
					accel));
	int32_t decelerate_steps = floor(
			estimate_acceleration_distance(block->nominal_rate, final_rate,
					-accel));
	...
	if ( plateau_steps < 0 ) {
		accelerate_steps = ceil(
				intersection_distance(initial_rate, final_rate, accel,
						block->step_event_count));
		accelerate_steps = max(accelerate_steps, 0); // Check limits due to numerical round-off
		accelerate_steps = min((uint32_t )accelerate_steps,
				block->step_event_count); //(We can cast here to unsigned, because the above line ensures that we are above zero)
		plateau_steps = 0;
	}
...
void Planner::reverse_pass_kernel(block_t* previous, block_t* current,
		block_t* next) {
	if ( !current )
		return;
	UNUSED(previous);
...
void Planner::forward_pass() {
	...
	for ( uint8_t b = block_buffer_tail; b != block_buffer_head; b =
			next_block_index(b) ) {
		block[0] = block[1];
		block[1] = block[2];
		...
		if ( current ) {
			...
			if ( current->recalculate_flag || next->recalculate_flag ) {
				...
				calculate_trapezoid_for_block(current,
						current->entry_speed / nom, next->entry_speed / nom);
...
void Planner::set_position_mm(const float& x, const float& y, const float& z,
	...
	long nx = position[X_AXIS] = lround(x * axis_steps_per_mm[X_AXIS]), ny =
			position[Y_AXIS] = lround(y * axis_steps_per_mm[Y_AXIS]), nz =
			position[Z_AXIS] = lround(z * axis_steps_per_mm[Z_AXIS]), ne =
...
void Planner::reset_acceleration_rates() {
	LOOP_XYZE(i)
		max_acceleration_steps_per_s2[i] = max_acceleration_mm_per_s2[i]
				* axis_steps_per_mm[i];
}
...
#else
	set_position_mm(current_position[X_AXIS], current_position[Y_AXIS],
