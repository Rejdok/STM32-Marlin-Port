 ...
 *
 */
/*todo ïðèäóìàòü, ÷òî ñ ýòèì äåëàòü*/
void serial_echopair_P(const char* s_P, uint8_t v) { serial_echopair_P(s_P, (int)v); }
void serial_echopair_P(const char* s_P, uint16_t v) { serial_echopair_P(s_P, (int)v); }
void serial_echopair_P(const char* s_P, int v) { serial_echopair_P(s_P, (int)v); }
void serial_echopair_P(const char* s_P, bool v) { serial_echopair_P(s_P, (int)v); }
void serial_echopair_P(const char* s_P, void *v) { serial_echopair_P(s_P, (unsigned long)v); }
...
/*Todo check maybe we don't need free memore in stm32*/
  int freeMemory() {
//    int free_memory;
//    if ((int)__brkval == 0)
...
void setup() {
  ...
  if (1) SERIAL_ECHOLNPGM(MSG_POWERUP);
#ifdef ARDUINO
...
/*TODO here i should stop interrupts*/
  thermalManager.disable_all_heaters();
